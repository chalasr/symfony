...
class ChainUserProvider implements UserProviderInterface, PasswordUpgraderInterface
{
    ...
    public function upgradePassword($user, string $newEncodedPassword): void
    {
        if (!$user instanceof PasswordAuthenticatedUserInterface) {
            trigger_deprecation('symfony/security-core', '5.3', 'The "%s()" method expects an instance of "%s" as first argument, you should make the "%s" class implement it.', PasswordUpgraderInterface::class, PasswordAuthenticatedUserInterface::class, get_debug_type($user));
        }
