    ...
    public function checkPassport(CheckPassportEvent $event): void
    {
        ...
        if ($passport instanceof UserPassportInterface && $passport->hasBadge(PasswordCredentials::class)) {
            ...
            if ($salt && !$user instanceof LegacyPasswordAuthenticatedUserInterface) {
                trigger_deprecation('symfony/security-http', '5.3', 'Returning a string from "getSalt()" without implementing the "%s" interface is deprecated. You should make the "%s" class implement it.', LegacyPasswordAuthenticatedUserInterface::class, get_debug_type($user));
            }
