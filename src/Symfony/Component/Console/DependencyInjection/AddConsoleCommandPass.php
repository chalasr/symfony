    ...
    public function process(ContainerBuilder $container)
    {
        ...
        foreach ($commandServices as $id => $tags) {
            ...
            if (!isset($tags[0]['command'])) {
                @trigger_error(sprintf('The "%s" tag requires a "command" attribute with the command name as value since version 3.4, omitting it (as for service "%s") is deprecated and will throw an exception in 4.0.', $this->commandTag, $id), E_USER_DEPRECATED);
            ...
            $commandMap[$commandName] = $commandRef = new TypedReference($id, $class);
            $aliases = array();
            ...
            foreach ($tags as $tag) {
                ...
                if (isset($tag['alias'])) {
                    $aliases[] = $tag['alias'];
                    $commandMap[$tag['alias']] = $commandRef;
                }
            }
        ...
        $container
            ...
            ->setArguments(array(ServiceLocatorTagPass::register($container, $commandMap), array_keys($commandMap)));
