...
class ConnectionTest extends TestCase
{
    ...
    public function testMaxEntries()
    {
        ...
        $connection = Connection::fromDsn('redis://localhost/queue?stream_max_entries=20000', ['delete_after_ack' => true], $redis); // 1 = always
        $connection->add('1', []);
    }
    ...
    {
        $this->expectDeprecation('Since symfony/redis-messenger 5.4: Not setting the "delete_after_ack" boolean option explicitly when using the redis transport is deprecated, its default value will change to true in 6.0.');

        Connection::fromDsn('redis://localhost/queue'); // 1 = always
    }
    ...
    public function testDeleteAfterReject()
    {
        ...
        $connection = Connection::fromDsn('redis://localhost/queue?delete_after_reject=true', ['delete_after_ack' => true], $redis); // 1 = always
        $connection->reject('1');
    }
