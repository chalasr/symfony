...
final class EntityValueResolver implements ValueResolverInterface
{
    ...
    private function getIdentifier(string $argumentName, InputInterface $input, MapEntity $options, ReflectionMember $member): mixed
    {
        if (\is_array($options->id)) {
            ...
            foreach ($options->id as $field) {
                ...
                $fieldName = (new \Symfony\Component\String\UnicodeString($field))->kebab()->toString();
        ...
        if ($options->id) {
            $idName = (new \Symfony\Component\String\UnicodeString($options->id))->kebab()->toString();
    ...
    private function getCriteria(string $argumentName, InputInterface $input, MapEntity $options, $manager, ReflectionMember $member): array
    {
        ...
        foreach (array_keys($mapping) as $attribute) {
            $attributeName = (new \Symfony\Component\String\UnicodeString($attribute))->kebab()->toString();
